...
abstract class DefaultFileService
{
    ...
    private static function getPathName($dir, $main_folder, $vip_folder): array
    {
        if (is_null($vip_folder)) {
            $path = $main_folder . '/';
            $full_path = $dir . $main_folder . '/';
        } else {
    ...
    {
        $path = self::getPathName($dir, $main_folder, $vip_folder)[0];
        $full_path = self::getPathName($dir, $main_folder, $vip_folder)[1];
